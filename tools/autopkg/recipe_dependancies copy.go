...
// // Recipe Dependency Resolution (ResolveRecipeDependencies)

// // Input: Recipe name, GitHub token flag, preferences path, dry run flag
// // Calls Search to find matching recipes
// // For each recipe match:

// // Verifies repository exists with VerifyRepoExists
// // Adds repository to list of repos to add
// // Parses the recipe file with ParseRecipeFile to extract identifier and dependencies
// // Adds all dependencies to a map

// // If not in dry run mode, adds all required repositories
// // Returns all dependencies as a slice

// // Search for Recipes (Search)

// // Input: Recipe name, GitHub token flag, preferences path
// // Applies rate limiting (6 seconds between requests)
// // Validates recipe name format (must be a recipe file or identifier)
// // If it's an identifier, converts it to a recipe name with ConvertRecipeIdentifierToName
// // Calls SearchRecipes to execute the autopkg search command
// // If search fails and the term has spaces:

// // Tries alternative search approaches (with quotes, app name only, original app name)

// // Parses search output with parseSearchOutput
// // Further filters results for recipes with spaces in name
// // Returns all matching recipes

// // Recipe Identifier Conversion (ConvertRecipeIdentifierToName)

// // Input: Recipe identifier (e.g., "com.github.username.AppName.download")
// // Splits identifier into parts
// // Tries to find app name and recipe type in the identifier
// // Uses heuristics to handle different identifier formats
// // Returns a recipe name (e.g., "AppName.download.recipe")

// // Repository Verification (VerifyRepoExists)

// // Input: Repository name, GitHub token flag
// // Constructs repository URL
// // Uses git command to check if repository exists
// // Returns boolean

// // Recipe File Parsing (ParseRecipeFile)

// // Input: Repository, path, GitHub token flag, preferences path
// // URL encodes the path
// // Builds raw GitHub URL for recipe content
// // Uses curl to fetch recipe file
// // Parses file based on extension (YAML or Plist)
// // Extracts identifier and parent recipe
// // If parent recipe exists:

// // Calls Search to find parent recipe matches
// // For each parent match, verifies repository and recursively calls ParseRecipeFile

// // Returns identifier and all dependencies

// // Search Output Parsing (parseSearchOutput)

// // Input: Output from autopkg search command
// // Finds header and separator in output
// // Determines column positions
// // Extracts repository and path for each match
// // Returns all valid matches

// // The process is recursive, with ParseRecipeFile potentially calling Search again for parent recipes, which in turn can lead to more parsing. Rate limiting is applied to avoid GitHub API limits, and there are various fallback mechanisms to handle recipe naming variations and search failures.
// // The current approach uses command-line calls to git, curl, and autopkg, with output parsing to extract needed information, rather than using APIs directly.
...
// 	var cmd *exec.Cmd
// 	if useToken {
// 		// If useToken is true, try to get the GitHub token from environment
// 		token := os.Getenv("GITHUB_TOKEN")
// 		if token != "" {
// 			cmd = exec.Command("curl", "-sL", "-H", fmt.Sprintf("Authorization: token %s", token), repoURL)
// 			logger.Logger("🔐 Using GitHub token for authentication", logger.LogDebug)
// 		} else {
// 			logger.Logger("⚠️ GitHub token requested but not found in environment", logger.LogWarning)
