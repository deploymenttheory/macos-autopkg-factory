...
func main() {
	...
	if *jsonOutput != "" {
		if err := exportResultsToJSON(result, *jsonOutput); err != nil {
...
// exportResultsToJSON exports the analysis results to a JSON file
func exportResultsToJSON(result *virustotal.SummaryResult, outputPath string) error {
	// Implementation would be similar to your existing ExportResultsToJSON function
	// For brevity, I'm not including the full implementation here

	fmt.Printf("📄 Exported VirusTotal results to JSON: %s\n", outputPath)
	return nil
}
